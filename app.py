...
def index():
    ...
    if request.method == "POST":
        ...
        for i in user_info:
            # Unhashing password and checking if it matches
            if(bcrypt.check_password_hash(i.password, user_password)):
                u_id = user_info[0].user_id
                # Redirecting to dashboard if password is correct
                return redirect(f'/{u_id}/{user_name}/dashboard')
...
def register():
    if request.method == "POST":
        ...
        if(validate(user_password) == "valid"):
            user_password_hashed = bcrypt.generate_password_hash(
                user_password).decode('utf-8')  # Hashing password
            add_user = User(user_name=user_name, first_name=first_name, last_name=last_name,
                            password=user_password_hashed)  # Adding user to database
...
def dashboardView(user_id, user_name):
    ...
    for tracker in tracker_list:
        # Getting tracker logs
        tlog = requests.get(
            f"{BASE}{user_id}/{str(tracker['tracker_id'])}/tracker_logs")
        tlogDict = tlog.json()
        if(len(tlogDict) > 0):
            last_log = tlogDict[-1]['tracker_timestamp']
            tracker['logs'] = last_log.split('.')[0]
        else:
            # If no logs found for a tracker, set it to 'No logs found'
            tracker['logs'] = 'No logs found'
...
def viewT(user_id, user_name, tracker_id, tracker_name):
    logL = requests.get(BASE + str(user_id)+"/" +
                        str(tracker_id) + "/tracker_logs")
    ...
    if(len(logListJson) > 0):
        ...
        if(tracker_type == "boolean"):
            ...
            if(true_value+false_value == 0):
                flag = 0
            plt.pie([true_value, false_value], labels=[
                    "True", "False"], shadow=True, startangle=90)
...
def addLog(user_id, user_name, tracker_id, tracker_name):
    ...
    for i in tracker_json:
        if(i['tracker_id'] == tracker_id):
            ...
            break
    tz = pytz.timezone('Asia/Kolkata')          # getting timezone
    tr = tz.localize(datetime.datetime.now())     # getting present time
...
def deleteLog(user_id, user_name, tracker_id, log_id, tracker_name):
    requests.delete(BASE + str(user_id)+"/" +
                    str(tracker_id) + "/tracker_log/" + str(log_id))
    return redirect(f'/{user_id}/{user_name}/{tracker_id}/{tracker_name}/tracker_logs')
